...
describe DiceSet do
  ...
  describe '#populate_count_hash' do
    it 'should populate hash with face counts correctly' do
      test_hash = { 1 => 3, 2 => 0, 3 => 1, 4 => 1, 5 => 1, 6 => 0 }
      allow(@dice).to receive(:roll).and_return([1, 1, 1, 3, 4, 5])
      expect(@dice.populate_count_hash(@dice.roll)).to eq(test_hash)
    end
  end
end
